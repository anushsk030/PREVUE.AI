...
router.get("/feedback-report/:interviewId", async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=feedback-report-${interviewId}.pdf`);
    ...
    doc.fillColor('#FFFFFF')
       .fontSize(28)
       .font('Helvetica-Bold')
       .text('Interview Feedback Report', 50, 50, { align: 'center' });

    // Interview details
    doc.fontSize(12)
       .font('Helvetica')
       .text(`Role: ${interview.role} | Mode: ${interview.mode} | Difficulty: ${interview.difficulty}`, 50, 90, { align: 'center' });
    
    const dateStr = new Date(interview.createdAt).toLocaleDateString('en-US', { 
      year: 'numeric', month: 'long', day: 'numeric' 
    });
    doc.text(`Date: ${dateStr}`, 50, 110, { align: 'center' });
    ...
    doc.fillColor('#1F2937')
       .fontSize(20)
       .font('Helvetica-Bold')
       .text('Overall Performance', 50, yPosition);
    ...
    // Score box
    doc.roundedRect(50, yPosition, 495, 80, 10)
       .fillAndStroke('#F3F4F6', '#E5E7EB');
    
    doc.fillColor('#4F46E5')
       .fontSize(48)
       .font('Helvetica-Bold')
       .text(`${totalScorePercentage}%`, 50, yPosition + 15, { align: 'center' });
    
    doc.fillColor('#6B7280')
       .fontSize(14)
       .font('Helvetica')
       .text('Total Score', 50, yPosition + 60, { align: 'center' });
    ...
    doc.fillColor('#1F2937')
       .fontSize(20)
       .font('Helvetica-Bold')
       .text('Detailed Scores', 50, yPosition);
    
    yPosition += 35;

    // Score categories with their values
    const scores = [
      { label: 'Correctness', value: interview.overallCorrectness || 0, color: '#10B981', valueIsPercent: false },
      { label: 'Depth', value: interview.overallDepth || 0, color: '#3B82F6', valueIsPercent: false },
      { label: 'Structure', value: interview.overallStructure || 0, color: '#8B5CF6', valueIsPercent: false },
      { label: 'Confidence', value: interview.confidence || 0, color: '#F59E0B', valueIsPercent: true },
      { label: 'Eye Contact', value: interview.eyeContact || 0, color: '#EC4899', valueIsPercent: true },
      { label: 'Stability', value: interview.stability || 0, color: '#14B8A6', valueIsPercent: true }
    ];

    // Draw score bars
    scores.forEach((score, index) => {
      const barY = yPosition + (index * 60);
      const scorePercentage = score.valueIsPercent
        ? Number(score.value || 0)
        : (Number(score.value || 0) * 10);
      const barWidth = (scorePercentage / 100) * 350;

      // Label
      doc.fillColor('#374151')
         .fontSize(14)
         .font('Helvetica-Bold')
         .text(score.label, 50, barY);

      // Background bar
      doc.roundedRect(50, barY + 20, 350, 20, 5)
         .fillAndStroke('#E5E7EB', '#D1D5DB');

      // Filled bar
      if (barWidth > 0) {
        doc.roundedRect(50, barY + 20, barWidth, 20, 5)
           .fill(score.color);
      }
      ...
      doc.fillColor('#1F2937')
        .fontSize(14)
        .font('Helvetica-Bold')
        .text(`${scorePercentage.toFixed(2)}%`, 420, barY + 20);
    });

     yPosition += scores.length * 60 + 30;

     // Questions and ideal answers
     doc.fillColor('#1F2937')
       .fontSize(20)
       .font('Helvetica-Bold')
       .text('Questions & Ideal Answers', 50, yPosition);
     ...
     questions.forEach((item, index) => {
      ...
      doc.fillColor('#111827')
        .fontSize(13)
        .font('Helvetica-Bold')
        .text(`Q${questionNumber}. ${questionText}`, 50, yPosition, { width: 495 });
      ...
      doc.fillColor('#374151')
        .fontSize(12)
        .font('Helvetica')
        .text(`Ideal Answer: ${idealAnswerText}`, 50, yPosition, { width: 495 });
    ...
    doc.fillColor('#6B7280')
       .fontSize(10)
       .font('Helvetica')
       .text('Generated by PREVUE.AI - Your AI Interview Preparation Partner', 50, yPosition, { align: 'center' });
